  ...
  return (
    <div className="bg-[#FDF7F0] min-h-screen py-6 px-4 md:px-8 lg:px-16 space-y-8">
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        {pins.map((pin) => {
          const mediaItems = [
            ...(pin.media?.video ? [{ type: "video", url: pin.media.video }] : []),
            ...(Array.isArray(pin.media?.images) ? pin.media.images.map((url) => ({ type: "image", url })) : []),
          ];
          const currentIndex = currentIndexes[pin._id] || 0;

          const handleSlide = (step) => {
            const newIndex = (currentIndex + step + mediaItems.length) % mediaItems.length;
            setCurrentIndexes((prev) => ({ ...prev, [pin._id]: newIndex }));
          };

          return (
            <div key={pin._id} className="bg-white rounded-xl shadow overflow-hidden">
              {/* Header */}
              <div className="p-4 flex items-center gap-4">
                ...
                />
                <div className="flex-1">
                  <h2 className="text-lg font-semibold text-gray-800">{pin.title}</h2>
                  <p className="text-xs text-gray-500">
                    {pin.owner?.name} · {new Date(pin.createdAt).toLocaleDateString()}
                  </p>
                </div>
                <FaFlag
                  onClick={() => setShowReport({ type: "pin", id: pin._id })}
              ...
              <div className="p-4 space-y-4">
                {pin.description && <p className="text-gray-700">{pin.description}</p>}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-6">
                    <button
                      onClick={() => handlePinReact(pin._id, pin.myReaction, "like")}
                      className={`flex items-center gap-1 ${pin.myReaction === "like" ? "text-blue-600" : "text-gray-500 hover:text-blue-600"}`}
                    >
                      <FaThumbsUp /> {pin.likes}
                    </button>
                    <button
                      onClick={() => handlePinReact(pin._id, pin.myReaction, "dislike")}
                      className={`flex items-center gap-1 ${pin.myReaction === "dislike" ? "text-red-600" : "text-gray-500 hover:text-red-600"}`}
                    >
                      <FaThumbsDown /> {pin.dislikes}
                    </button>
                  </div>
                    ...
                    {pin.comments.map((c) => (
                      <div key={c.id || c._id} className="space-y-1">
                        ...
                        <div className="flex items-center gap-6 text-gray-500 text-sm pl-11">
                          <button
                            onClick={() => handleCommentReact(pin._id, c.id, c.myReaction, "like")}
                            className="flex items-center gap-1 hover:text-blue-600"
                          >
                            <FaThumbsUp size={16} /> {c.likes}
                          </button>
                          <button
                            onClick={() => handleCommentReact(pin._id, c.id, c.myReaction, "dislike")}
