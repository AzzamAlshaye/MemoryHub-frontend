...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    console.log("Current User ID:", currentUserId);
        return pinService.list();
      })
      .then(pins => {
        console.log("All Pins:", pins);

        const ownPins = pins.filter(pin => {
          const ownerId = String(pin.owner?._id || pin.owner);
          return ownerId === currentUserId;
        });

        console.log("Own Pins:", ownPins);
        setMemories(ownPins);
...
{/* memories sections */}
        <section className="flex-1 mb-10">
          ...
          <div className="grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {memories.length === 0 && <p className="text-gray-500">No memories to display.</p>}
            {memories.map(memory => (
              <div key={memory._id}
                onClick={() => handleEditPin(memory)}
